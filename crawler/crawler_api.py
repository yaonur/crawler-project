...
class Crawler:
    ...
    async def _request(self, url):
        async with httpx.AsyncClient() as client:
            try:
                self.logger.info(f"starting request for {url}")
            ...
            # of company requirements
                ...
                self.logger.exception(
                    "Error HTTP: %s code:%s", url, errhttp
                )
                return False
                ...
                self.logger.exception(
                    "Error HTTP: %s code:%s", url, errconn
                )
                self.logger.error("adding to skipped urls")
                self.skipped_urls.add(url)
                return False
                ...
                self.logger.exception("Error getting url: %s code: %s", url,e)
                return False
    ...
    async def add_workers(self, num_workers=1):
        for i in range(num_workers):
            self.logger.debug(f"Adding worker")
    ...
    async def crawl(self, sleep_time=0.1):
        while True:
            ...
            self.logger.info("Worker getting url from queue>>>>>")
            url = await self.url_queue.get()
            self.logger.info(f"Worker have url from queue: {url}")
            ...
            try:
                ...
                self.logger.error("--------------------------------------" )
                self.logger.exception("Error getting url on the main _request url: %s", e)
            ...
            try:
                ...
                self.logger.error("--------------------------------------" )
                self.logger.exception("Error on parsing soup  url ")
                self.must_handle_urls.add(url)
                self.url_queue.task_done()
                continue
            for link in links:
            ...
            should_add_workers = int(queue_size/2) > num_workers and num_workers<self.max_worker_count
